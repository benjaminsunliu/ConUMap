...
export default function RoutesInfoPopup({ routes, isOpen, onRouteSelect, onStepSelect, onBack }: Props) {
    ...
    const header = useMemo(() => {
        const selectorOptions = availableTransports.map((transport, index) => {
            const shuttleIcon = (
                <Image source={colorScheme === "dark" ? ShuttleIconDark : ShuttleIconLight} style={{ width: 75, height: 40 }} />
            );
            const defaultIcon = (
                <Ionicons
        ...
        return (
            <>
                {onBack && (
                    <TouchableOpacity onPress={onBack} style={styles.headerBackButton} testID="routes-back-button">
    ...
    return (
        <InfoPopup shouldDisplay={isOpen} header={header} testID="routes-info-popup">
            {selectedRoute ? (
                <>
                    <TouchableOpacity style={styles.backButton} onPress={() => setSelectedRoute(null)} testID="back-to-routes-button">
                        ...
                        <RouteStep
                            ...
                            onPress={() => {
                                ...
                                if (encoded && onStepSelect) {
                                    onStepSelect(encoded, step?.travel_mode ?? "WALK", step?.transit_details?.line?.vehicle_type);
                                }
                            }}
                        />
                    ))}
        ...
        <TouchableOpacity style={styles.overviewContainer} onPress={() => onRouteSelect(route)} testID={testID}>
            ...
            {route?.legs[0]?.arrival_time && route?.legs[0]?.departure_time ? (
                <Text style={styles.routeInfo}>
                    From {route?.legs[0]?.departure_time.text} To {route?.legs[0]?.arrival_time.text}
                </Text>
            ) : null}
