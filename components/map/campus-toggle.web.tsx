...
export default function CampusToggle({ mapRef, viewRegion }: Readonly<Props>) {
    ...
    const pendingProgrammaticValueRef = React.useRef<number | null>(null);
    const suppressRegionSyncUntilRef = React.useRef(0);
    ...
    useEffect(() => {
        if (Date.now() < suppressRegionSyncUntilRef.current) {
            return;
        }
        ...
        if (newSwitchValue !== switchValue) {
            pendingProgrammaticValueRef.current = newSwitchValue;
            setSwitchValue(newSwitchValue);
        }
    }, [viewRegion, switchValue]);

    const focusCampusOnPress = (value: number) => {
        // Ignore callback invocations caused by controlled value syncs.
        if (pendingProgrammaticValueRef.current === value) {
            pendingProgrammaticValueRef.current = null;
            return;
        }

        suppressRegionSyncUntilRef.current = Date.now() + 600;
