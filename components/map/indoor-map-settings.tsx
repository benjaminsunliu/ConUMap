...
export default function MapSettings({ wheelchairOnly, setWheelchairOnly, poiFilters, setPoiFilters }: MapSettingsProps) {
    ...
    return (
        <View style={styles.container}>
            <TouchableOpacity style={styles.fab} onPress={() => setOpen((prev) => !prev)} activeOpacity={0.8}>
            ...
            {open && (
                <View style={styles.panel}>
                    ...
                    <View style={styles.row}>
                        <Ionicons name="accessibility-outline" size={18} color={theme.mapSettings.icon} />
                        <Text style={styles.label}> Wheelchair Accessible </Text>
                        <Switch
                            value={wheelchairOnly}
                            onValueChange={setWheelchairOnly}
                            thumbColor={
                                wheelchairOnly ? theme.mapSettings.toggleTrue : theme.mapSettings.toggleFalse
                            }
                            trackColor={{ true: theme.mapSettings.toggleTrue, false: theme.mapSettings.toggleFalse }}
                        />
                    </View>
                    ...
                    {pois.map((item) => (
                        <TouchableOpacity
                            ...
                            onPress={() =>
                                togglePOI(
                                    item.key as keyof typeof poiFilters
                                )
                            }
                        >
                            <Ionicons name={item.icon as any} size={18} color={theme.mapSettings.icon} />
